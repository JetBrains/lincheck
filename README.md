...
6) Copy the report outside the container and exit the image.

`$ cp -r ./build/reports/tests/test/. /report`

`$ exit`

7) Open the tests execution report located at `./report/index.html` (if you used Docker) or `build/reports/tests/test/index.html` (if you didn't use Docker) in any browser.
...
* `AbstractQueueSynchronizerTest` (the framework for almost all synchronization primitives in Java)
...
**Test:** `ConcurrentLinkedDequeTest.kt`  
...
**Source:** Java Standard Library (still present in the latest JDK version).  
**Test report:** `AbstractQueueSynchronizerTest.kt`
...
**Test:** `NonBlockingHashMapLongTest.kt`
...
**Tests:** `ConcurrentSuffixTreeTest.kt`, `ConcurrentRadixTreeTest.kt`
...
**Test:** `LogicalOrderingAVLTest.kt`
...
**Test:** `ConcurrencyOptimalMapTest.kt`
