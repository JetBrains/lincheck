...
class Client(val env: Environment<Message, Unit>) : Node<Message> {
    ...
    override fun onMessage(message: Message, sender: Int) {
        check(sender == server) {
            "Only server can send messages to clients"
        }
        when (message) {
...
class SequentialSpecification {
    ...
    suspend fun put(key: Int, value: Int) = map.put(key, value)

    suspend fun get(key: Int) = map[key]
}
...
class Test {
    ...
    fun test() {
        DistributedOptions<Message, Unit>()
            ...
            .nodeType(Server::class.java, 1, canFail = false)
            .nodeType(Client::class.java, 3)
            //.setMaxNumberOfFailedNodes { it - 1 }
            //.crashMode(CrashMode.ALL_NODES_RECOVER)
            .storeLogsForFailedScenario("simple_example.txt")
            .check()
    }
}*/