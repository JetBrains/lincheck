...
import org.jetbrains.kotlinx.lincheck.InternalLincheckException
...
class UnexpectedExceptionTest : AbstractLincheckTest(UnexpectedExceptionFailure::class) {
    ...
    @Operation
    fun operation2() {
        if (canEnterForbiddenSection) throw InternalLincheckException
    }
...
class CoroutineResumedWithUnexpectedExceptionTest : AbstractLincheckTest(UnexpectedExceptionFailure::class) {
    ...
    suspend fun operation() {
        suspendCancellableCoroutine<Unit> { cont ->
            cont.resumeWithException(InternalLincheckException)
        }
    }
