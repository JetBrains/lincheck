...
internal class LogQueueTest :
    AbstractNVMLincheckTest(Recover.DETECTABLE_EXECUTION, THREADS_NUMBER, SequentialQueue::class) {
...
internal class LogQueue(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
...
internal class LogQueueFailingTest15 : LogQueueFailingTest() {
    override val q = LogFailingQueue15(THREADS_NUMBER + 2)
}

internal class LogQueueFailingTest16 : LogQueueFailingTest() {
    override val q = LogFailingQueue16(THREADS_NUMBER + 2)
}

internal class LogQueueFailingTest17 : LogQueueFailingTest() {
    override val q = LogFailingQueue17(THREADS_NUMBER + 2)
}

internal class LogQueueFailingTest18 : LogQueueFailingTest() {
    override val q = LogFailingQueue18(THREADS_NUMBER + 2)
}

internal class LogQueueFailingTest19 : LogQueueFailingTest() {
    override val q = LogFailingQueue19(THREADS_NUMBER + 2)
}

internal class LogFailingQueue1(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
...
internal class LogFailingQueue2(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
...
internal class LogFailingQueue3(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
...
internal class LogFailingQueue4(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
...
internal class LogFailingQueue5(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
...
internal class LogFailingQueue6(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
...
internal class LogFailingQueue7(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
...
internal class LogFailingQueue8(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
...
internal class LogFailingQueue9(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
    ...
    override fun recover() {
        ...
        while (v !== null) {
            ...
            if (v.logRemove.value !== null) {
                v.logRemove.flush()
                v.logRemove.value!!.node.value = v
                // here should be v.logRemove.value!!.node.flush()
            }
            v.next.flush()
            v = v.next.value
        }
...
internal class LogFailingQueue10(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
...
internal class LogFailingQueue11(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
...
internal class LogFailingQueue12(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
...
internal class LogFailingQueue13(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
...
internal class LogFailingQueue14(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
    ...
    override fun pop(threadId: Int, operationNumber: Int): Int? {
        ...
        while (true) {
            ...
            if (first === last) {
                ...
                if (nextNode.logRemove.compareAndSet(null, log)) {
                    first.next.value!!.logRemove.flush()
                    // here should be nextNode.logRemove.value!!.node.value = first.next.value
                    nextNode.logRemove.value!!.node.flush()
                    head.compareAndSet(first, nextNode)
                    return currentValue
                } else {
...
internal class LogFailingQueue15(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
            ...
            if (first === last) {
                ...
                if (nextNode.logRemove.compareAndSet(null, log)) {
                    ...
                    if (head.value === first) {
                        first.next.value!!.logRemove.flush()
                        nextNode.logRemove.value!!.node.value = first.next.value
                        // here should be nextNode.logRemove.value!!.node.flush()
                        head.compareAndSet(first, nextNode)
                    }
                }
            }
...
internal class LogFailingQueue16(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
...
internal class LogFailingQueue17(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
...
internal class LogFailingQueue18(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
...
internal class LogFailingQueue19(threadsNumber: Int) : RecoverableQueue {
    private val dummy = QueueNode(42)
    private val head = nonVolatile(dummy)
    private val tail = nonVolatile(dummy)
    private val logs = List(threadsNumber) { nonVolatile(null as LogEntry?) }
    ...
    override fun recover() {
        ...
        while (v !== null) {
            ...
            v.logInsert.value?.status?.flush()
            if (v.logRemove.value !== null) {
                // here should be v.logRemove.flush()
                v.logRemove.value!!.node.value = v
                v.logRemove.value!!.node.flush()
            }
            v.next.flush()
