        ...
        val res = when (type) {
            ...
            CANCELLATION -> {
                ...
                if (!actor.promptCancellation) {
                    check(wasSuspended) {
                        "The operation can be cancelled after resumption only in the prompt cancellation mode"
                    }
                }
                if (resumedOperations.remove(ticket) != null) {
                    check(actor.promptCancellation) {
