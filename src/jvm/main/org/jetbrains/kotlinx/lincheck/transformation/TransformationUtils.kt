...
/**
 * Pops arguments of the method except the receiver.
 *
 * Before execution:
 * STACK: param_1, param_2, ... param_n
 *
 ...
 * @param methodDescriptor String representation of the method's descriptor.
 */
internal fun GeneratorAdapter.popArguments(methodDescriptor: String) {
    val argumentTypes = getArgumentTypes(methodDescriptor)
    // Pop values in reverse order
    for (i in argumentTypes.indices.reversed()) {
        val valueType = argumentTypes[i]
        if (valueType == DOUBLE_TYPE || valueType == LONG_TYPE) {
            pop2()
        } else {
            pop()
        }
...
/**
 * Duplicates the value of some type.
 *
 * Before execution:
 * STACK: VALUE
 *
