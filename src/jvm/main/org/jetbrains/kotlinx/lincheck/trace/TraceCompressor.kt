...
/**
 * Compresses trace node for `default` and `access` functions.
 * For more details check [isDefaultPair] and [isAccessPair].
 */
private fun SingleThreadedTable<TraceNode>.compressDefaultAndAccessPairs() = compressNodes { node ->
    val singleChild = if (node.children.size == 1) node.children[0] else return@compressNodes node
    if (node !is CallNode || singleChild !is CallNode) return@compressNodes node
    if (!isDefaultOrAccessPair(node.tracePoint.methodName, singleChild.tracePoint.methodName)) return@compressNodes node

    // Combine fields of next and current, and store in current
    node.tracePoint.methodName = singleChild.tracePoint.methodName
    node.tracePoint.parameters = singleChild.tracePoint.parameters

    check(node.tracePoint.returnedValue == singleChild.tracePoint.returnedValue)
    check(node.tracePoint.thrownException == singleChild.tracePoint.thrownException)

    val newNode = node.copy()
    singleChild.decrementCallDepthOfTree()
    singleChild.children.forEach { newNode.addChild(it) }
    newNode
}
...
private fun isDefaultOrAccessPair(currentName: String, nextName: String): Boolean =
