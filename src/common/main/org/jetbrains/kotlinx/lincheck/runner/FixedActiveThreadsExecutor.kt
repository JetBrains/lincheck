...
internal expect class LockSupport {
    companion object {
        fun park()
...
internal class FixedActiveThreadsExecutor(private val nThreads: Int, runnerHash: Int) {
    ...
    private fun getResult(iThread: Int, deadline: Long): Any {
        ...
        val currentThread = TestThread.currentThread()
        if (results[iThread].compareAndSet(null, TestThread.currentThread())) {
    ...
    private fun getTask(iThread: Int): Any {
        ...
        val currentThread = TestThread.currentThread()
        if (tasks[iThread].compareAndSet(null, TestThread.currentThread())) {
    ...
    private fun setResult(iThread: Int, any: Any) {
        ...
        val thread = results[iThread].value
