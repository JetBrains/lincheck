...
object CompressingPostprocessor : TracePostprocessor {
    ...
    private fun TRTracePoint.compressSyntheticFieldAccess(reader: LazyTraceReader): TRTracePoint {
        ...
        val newNode = when (actualNode) {
            is TRReadFieldTracePoint -> TRReadFieldTracePoint(reader.context, actualNode.threadId, codeLocationId, actualNode.fieldId,
                                                    actualNode.obj, actualNode.value, actualNode.eventId)
            is TRWriteFieldTracePoint -> TRWriteFieldTracePoint(reader.context, actualNode.threadId, codeLocationId, actualNode.fieldId,
                                                      actualNode.obj, actualNode.value, actualNode.eventId)
            else -> return this
        }
        // We don't have containers here, so don't save sub-tree status
        newNode.copyDiffStatus(this)
    ...
    private fun TRTracePoint.compressAutoGeneratedFieldAccess(reader: LazyTraceReader): TRTracePoint {
        ...
        // Note: when constructing new trace points (TRReadFieldTracePoint or TRWriteFieldTracePoint),
        // for proper clicking in the trace recorder plugin sidebar to work,
        // code location must be rewritten with the one from the parent trace point
        ...
        if (events.size == 1) {
            val isAutoGeneratedFieldGetter = (
                child1 is TRReadFieldTracePoint && className == child1.className &&
            ...
            if (isAutoGeneratedFieldGetter) {
                return TRReadFieldTracePoint(
                    context = reader.context,
                    threadId = child1.threadId,
                    codeLocationId = codeLocationId,
                    fieldId = child1.fieldId,
                    obj = child1.obj,
                    value = child1.value,
                    eventId = child1.eventId
                ).also { it.copyDiffStatus(this) }
            }
        }
        else /* events.size == 2 */ {
            ...
            val isAutoGeneratedFieldSetter = (
                ...
                child2 is TRWriteFieldTracePoint && className == child2.className &&
                isAutoGeneratedFieldSetterName(methodName, child2.name)
            )

            if (isAutoGeneratedFieldSetter) {
                return TRWriteFieldTracePoint(
                    context = reader.context,
                    threadId = child2.threadId,
                    codeLocationId = codeLocationId,
                    fieldId = child2.fieldId,
                    obj = child2.obj,
                    value = child2.value,
                    eventId = child2.eventId
                ).also { it.copyDiffStatus(this) }
            }
        }
